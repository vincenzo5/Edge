...
| Snap vertical crosshair to bar | **Done** | Vertical line snaps to the center of the bar under the cursor (TradingView-style); horizontal line still follows cursor Y |
...
| Long Position (`longPosition`) | `long_position` | **Done** — click-to-place at cursor (one-point); green target / red stop box; TV-style 4-handle resize (TP/stop vertical, entry-left 2-axis, right width-only) with stop/target clamped to valid side of entry; target/entry/stop labels; left-edge 1R ticks + in-box NR labels in profit zone; empty-margin near-miss snaps to last bar (no `timestamp: 0` stretch); **stick entry to last price** OFF by default (Settings opt-in; stop/TP fixed while enabled; manual entry drag turns stick off) |
| Short Position (`shortPosition`) | `short_position` | **Done** — click-to-place at cursor (one-point); red stop / green target box; TV-style 4-handle resize (TP/stop vertical, entry-left 2-axis, right width-only) with stop/target clamped to valid side of entry; target/entry/stop labels; left-edge 1R ticks + in-box NR labels in profit zone; empty-margin near-miss snaps to last bar (no `timestamp: 0` stretch); **stick entry to last price** OFF by default (Settings opt-in; stop/TP fixed while enabled; manual entry drag turns stick off) |
...
| Journal trade chart forks | **Done** | Attach editable live chart fork to journal trade: chart snapshot menu **Attach to journal trade…**, trade detail **Capture active chart** (instant fork), or Screenshots **Capture chart** (popup markup studio → PNG + fork); stores deep-cloned `CellConfig` + screenshot + optional plan levels; **Open trade chart** modal with live feed, entry/exit markers, debounced save, reset-to-capture; fork isolated from workspace (`journal_trade_chart_snapshots`, IndexedDB fallback) |
...
| `setMagnet` | **Done** | OHLC snap during create, CP edit, and whole-tool drag |
