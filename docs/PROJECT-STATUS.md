...
| Trade management playbook — Phase 6 | Optional notify-only alert bundles at manage levels on attach; alertBundleId linkage + detach cleanup | **Active** | pending | manageNotifyAlerts.ts, migration 0036, tradingService.ts, ManagePlaybookPicker.tsx, TradeOrderForm.tsx, ProtectiveOcoForm.tsx, alertRepository.ts, alertClient.ts |
