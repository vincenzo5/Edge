...
## Task Contract — Journal qty + Flex TZ fix

- **Status:** **Passing** (2026-08-06) — deploy to local prod in progress
- **Goal:** Closed-trade journal qty uses full open size (not max exit fill); Flex fill times import as America/New_York→UTC; rebuild preserves `initialStop`; repair existing edge_prod Flex rows.
- **Verification:** Focused 58 tests; edge_prod Flex hour-band repair; LQDA stop 77.57 → ~1.0R after qty fix. Evidence: `docs/evidence/journal-flex-time-repair-2026-08-06.txt`, `docs/evidence/journal-prod-trade-audit-2026-08-06.txt`
- **Blockers:** none
- **Next:** Promote commit to local prod so ingest stops wiping qty/stop
