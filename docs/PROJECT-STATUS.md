...
- **Next best step:** Commit journal qty + Flex TZ fix, then `local:prod:container:deploy` — old prod image still rebuilds closed trades with `netQuantity=0` and wipes the repair on ingest.
## Startup Readiness
