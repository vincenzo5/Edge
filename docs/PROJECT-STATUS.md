...
|---------|----------|-------|-------------------------------------|-------|
| Runtime interaction performance — Phase 1 | Drawing hover/select/drag and crosshair-only updates do not rebuild candles+indicators series composite | **Active** | pending — remove drawings/selection from SERIES_INVALIDATING; focused tests + harness vs Phase 0 | packages/chart-react/src/engine/renderScheduler.ts,packages/chart-react/src/engine/layers.ts,packages/chart-react/src/engine/renderScheduler.test.ts,packages/chart-react/src/engine/layers.test.ts,src/lib/chart/ARCHITECTURE.md |
