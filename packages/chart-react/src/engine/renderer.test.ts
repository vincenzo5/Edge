...
import type { Candle, VisibleRange, Theme } from '@edge/chart-core/contracts';
import { PRICE_AXIS_WIDTH, TIME_AXIS_HEIGHT, plotWidth, plotHeight } from '@edge/chart-core/layout';
...
describe('getColors', () => {
  it('returns dark palette for dark theme', () => {
    ...
    expect(c.up).toBe('#2dd4a8');
    expect(c.grid).toBe('#171f2e');
    expect(c.axisBg).toBe('#000000');
  });
});
