import type { Candle, IndicatorConfig, Interval, Theme } from '@edge/chart-core';
import type { ChartSettings, RequiredChartSettings } from './chartSettings';
import {
  mergeChartSettings,
  resolvePriceDecimals,
} from './chartSettings';
import { formatChange, formatPrice, formatVolume } from '@edge/chart-core/format';
...
export type { LegendSection, LegendValueEntry, LegendActionIcon, SeriesOutput, SeriesColor, PlotKind } from '@edge/chart-core/legend/types';
...
export function resolvePriceLegend(
  ...
  if (settings.statusLine.showBarChangeValues) {
    sections.push({
      ...
      label: '',
      value: changeValue,
      color: isUp ? 'var(--edge-positive)' : 'var(--edge-negative)',
