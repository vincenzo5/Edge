...
**Warmup:** `/api/market-data/warmup` is best-effort and bounded. TWS sidecar `/warmup` pre-subscribes quote symbols; **batch quote fetch is client-owned** via `MarketDataProvider` (no `getQuotes` phase in warmup). Active chart cell candles run first, then **active-symbol market context** is prefetched (`market_context` phase) so the legend breadcrumb hits warmed server cache, then secondary cells in parallel; options expirations defer when Massive/TWS/IBKR are unavailable (options have no Yahoo fallback). Client warmup POSTs are coalesced per layout key in `MarketDataProvider` to avoid duplicate waves on Strict Mode remount or symbol-universe churn. When Massive is configured, options warmup uses Massive and does not require broker login.
...
**Sidecar ownership:** Docker Compose owns the sidecar beside both Gateways (`restart: unless-stopped`) — see [Persistent TWS Sidecar Roadmap](../../../docs/roadmaps/persistent-tws-sidecar-roadmap.md). `npm run ib:gateway:up` starts Gateways + sidecar together. Host `npm run tws:sidecar` is emergency/fallback only (stop Compose sidecar first if port `:8765` conflicts).
...
2. If the sidecar is unreachable and `TWS_MANAGED=local`, Edge spawns `scripts/tws-sidecar.sh` with `TWS_MANAGED_BY=edge-local` (boot-time auto-spawn is local-only). If `TWS_MANAGED=external`, Recover never spawns — the UI surfaces an operator restart message (`npm run ib:gateway:up`; emergency: `npm run tws:sidecar`).
...
4. When the IB worker is wedged or the API client ID is stuck, `TWS_MANAGED=local` restarts the Next-owned sidecar before retrying reconnect; `TWS_MANAGED=external` fails closed with the operator restart message. Stale `clientId` after restart surfaces a manual action: restart IB Gateway or change `TWS_CLIENT_ID`.
...
```bash
npm run ib:gateway:up       # primary — Gateways + Compose sidecar
npm run tws:sidecar-setup   # create Python venv + install ib_insync (emergency host path)
npm run tws:sidecar         # emergency fallback — stop Compose sidecar first if :8765 busy
