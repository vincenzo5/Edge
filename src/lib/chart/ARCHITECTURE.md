...
- **Drawings**: register in `drawings/registry.ts`; toolbar names aliased in `pluginHost.ts`. Utility tools include `measure` (bar/price line), `ruler` (shaded Δtime/Δprice band; ⇧+click shortcut on price pane), and `risk_ruler`. Forecasting tools `long_position` / `short_position` use `createPositionPlugin()` — instant place on toolbar select at last-bar close with left edge on the last bar (default stop/TP/width from Settings default policy Geometry when set, else 2R target; still resizable via TradingView-style 4 handles: target/stop vertical-only (`ns-resize`); entry-left moves entry + left edge (`move`); right edge width-only (`ew-resize`)), profit/loss zones, left-edge 1R yard lines with in-box NR labels, TV-style target/entry/stop in-box labels (selected/hovered) backed by `risk/*` helpers, and price-axis badges at entry/stop/take-profit via `axisAnnotations` (respects Scales → Drawing price labels). Manual entry edits disable stick-to-last-price. Trade panel policy apply can reshape bound drawing target via `ActiveChartCommands.reshapePositionDrawing`.
