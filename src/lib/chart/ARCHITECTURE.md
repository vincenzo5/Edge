...
**Magnet mode:** When enabled, `plotToPoint` snaps price to the nearest OHLC on the hovered candle (price pane). Whole-tool drag snaps the plugin's `magnetAnchorIndex` point (default `0`; long/short use entry) and translates other anchors rigidly via `translateDrawingPoints`. Control-point drag pre-snaps the pointer then reshapes through `updateFromControl`.
