...
StockApp → PrimaryChartBrowserTabQuote (document title + favicon)
         → ChartGrid ── ChartDrawingRail (multi-pane; targets active cell)
              └─ ChartCell → EdgeChart
                                ...
                                ├─ CrosshairOverlay
                                ├─ HistoryNavigatorOverlay (transient pan/zoom position chrome)
...
| `packages/chart-core/src/drawings/position_tool.ts` | Shared long/short position plugin factory; geometry in `positionGeometry.ts`; profit-zone 1R yard lines; labels via `risk/positionLabels.ts`; one-point click-to-place; `styles.stickEntryToLastPrice` (default OFF, Settings opt-in) optionally sticks entry to live last price |
...
- **Drawings**: register in `drawings/registry.ts`; toolbar names aliased in `pluginHost.ts`. Utility tools include `measure` (bar/price line), `ruler` (shaded Δtime/Δprice band; ⇧+click shortcut on price pane), and `risk_ruler`. Forecasting tools `long_position` / `short_position` use `createPositionPlugin()` — one-point click-to-place at cursor (default stop/TP/width from click-bar range and Settings default policy Geometry when set, else 2R target; entry stays fixed after place; still resizable via TradingView-style 4 handles: target/stop vertical-only (`ns-resize`); entry-left moves entry + left edge (`move`); right edge width-only (`ew-resize`); CP drag clamps stop/target to valid side of entry), profit/loss zones, left-edge 1R yard lines with in-box NR labels, TV-style target/entry/stop in-box labels (selected/hovered) backed by `risk/*` helpers, and price-axis badges at entry/stop/take-profit via `axisAnnotations` (respects Scales → Drawing price labels). Stick entry to last price is OFF by default (Settings opt-in); manual entry edits disable stick when enabled. Trade panel policy apply can reshape bound drawing target via `ActiveChartCommands.reshapePositionDrawing`.
...
**Magnet mode:** When enabled, strong magnet snaps drawing points to the nearest OHLC on the candle under the cursor (price pane). `resolveMagnetDragPlot` applies role-aware CP drag: `price` keeps handle X and snaps Y; `time` snaps bar X; `xy` snaps both (free points). Long/short left handles (target, entry, stop) use price-only under magnet so the left edge does not jump horizontally. Whole-tool drag snaps the plugin's `magnetAnchorIndex` point (default `0`; long/short use entry) and translates other anchors rigidly via `translateDrawingPoints`.
...
**Layout persistence fan-out (Phase 7):** Per-cell layout slices live in `cellLayoutStore` (`useCellLayoutConfig` / `useSyncExternalStore` per `chartId`). Drawing/viewport-only edits write the keyed store + debounced flush into `workspaceTabs` without waking sibling `ChartCell`s. Shell updates (symbol/interval/indicators) eagerly `setCellConfig` for all visible cells in `useStockAppLayoutController.applyCellUpdate` so legends read the same symbol as `layout.cells` without waiting on post-commit mirror. `DrawingStore.revision` and `layoutRevisionFingerprint` replace hot-path `JSON.stringify` equality for drawing persist and dirty keys.
