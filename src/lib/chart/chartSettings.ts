...
import type { PriceScaleType } from './priceScaleTransform';
...
export type { ChartTimeZone } from './timeZone';
export { DEFAULT_CHART_TIMEZONE } from './timeZone';

export type { CrosshairMode } from './crosshairMode';
export type { PriceScaleType } from './priceScaleTransform';
...
export function migrateChartSettings(partial?: ChartSettings | null): GroupedChartSettings {
  ...
  const { symbol, statusLine, scales, canvas, trading, ...rest } = partial;
  const grouped: GroupedChartSettings = {
    symbol,
    statusLine,
    scales,
