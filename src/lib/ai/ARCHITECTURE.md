...
**Quant research runtime (Phase 4 — sandboxed Python cells):** Server-side experiment compute ships dataset materialization, descriptive profile jobs, signal / event studies, vectorized strategy evaluation, and bounded Python research cells via `src/lib/researchCompute/` and server-only registry tools in `src/lib/ai/tools/researchCompute.ts` (`create_research_dataset`, `get_research_dataset`, `profile_research_dataset`, `run_signal_study`, `run_strategy_evaluation`, `run_research_code`, `cancel_research_job`, `get_research_job`, `get_research_artifact`). Tools are **not** in `CLIENT_AI_TOOLS`; permission `read`; inputs are dataset refs + typed specs — never raw OHLCV from the model. `run_research_code` executes in a local Docker worker (`--network=none`, read-only dataset mount, allowlisted Polars/DuckDB/NumPy/SciPy). `ResearchComputePort` is injected in `createServerToolContext()`. Compact tool results feed Copilot Data blocks (`researchProfile` hint); source, stdout, and full series stay in artifacts under `data/research/`. Domain: [src/lib/researchCompute/ARCHITECTURE.md](../researchCompute/ARCHITECTURE.md). Roadmap: [docs/roadmaps/quant-research-runtime-roadmap.md](../../../docs/roadmaps/quant-research-runtime-roadmap.md).
