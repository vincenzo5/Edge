...
**Phase 8 behavior:** Empty Copilot hero uses idle placeholder rotator (`COPILOT_IDLE_QUESTIONS`) — workflow prompt chips were removed from the empty hero (Talk empty UX). Curated workflow prompts (`COPILOT_WORKFLOW_PROMPTS`) surface as **Follow-up** chips under the latest completed assistant turn (chat blocks Phase 4); chip click sends via the normal chat path with workspace snapshot. `prepare_chart_for_analysis` requires confirmation (clears drawings). `get_chart_state` and `summarize_chart` include slim `dataProvenance` from active chart `dataMeta`. System prompt instructs the model to cite provider source/freshness when present. Rich-annotations playbooks (Phase D) remain deferred — `playbookId` reserved only.
...
**Copilot chat blocks (Phase 1 — scroll + Action shell):** `CopilotMessageList` stick-to-bottom policy from `chatScrollPolicy.ts` is verified under stream growth when unpinned (no viewport yank). Pending confirms render through shared `CopilotActionBlock` fed by `toolStepToActionBlock` — Accept/Reject testids and `onResolveConfirm` + `confirmationToken` wiring unchanged. Trace (Thoughts) and artifact cards unchanged.

**Copilot chat blocks (Phase 2 — Media + Data shells):** Generic `CopilotMediaBlock` and `CopilotDataBlock` render in-thread tool products and user attachments. Chart / screener / journal `artifactHint`s map through `hintToBlockSketch` + `toolStepToMediaBlock` / `toolStepToDataBlock` (chart → Media caption + Open; screener/journal → Data kv + Pin). User attachments reuse `attachmentToMediaBlock`. Pin / Open wiring preserved via existing `onPinArtifact` and `onOpenHref` (router push). NDJSON stream union unchanged; blocks remain in-memory only. Thin `CopilotArtifactCard` remains fallback for note/aiCallout hints.

**Copilot chat blocks (Phase 3 — Reference chips):** `CopilotReferenceBlock` renders compact chips under assistant turns via `toolStepsToReferenceBlock` — chart `artifactHint`s and done `REFERENCE_TOOL_NAMES` steps (summary parse fallback). Chips open chart deep links through existing `onOpenHref`; overflow collapses at 4 visible (+N expand). No Context banner; NDJSON stream unchanged; blocks in-memory only.

**Copilot chat blocks (Phase 4 — Follow-ups):** `CopilotFollowupsBlock` renders curated workflow chips under the **latest completed** assistant turn via `workflowPromptsToFollowupsBlock` (`COPILOT_WORKFLOW_PROMPTS`). Chip click calls `handleSend(prompt, [])` with workspace snapshot — same path as composer send. Hidden while streaming or pending confirm; empty hero stays pill-free. NDJSON stream unchanged; blocks in-memory only.

**Copilot chat blocks (Phase 5 — Enrich Action payloads):** `ActionChatBlock` accepts optional `summaryRows` (kv pairs, max 12). `actionSummaryRowsFromStep` derives rows from in-memory `confirmArguments` for `place_order` (draft fields), `attach_playbook`, `delete_drawing`, and `prepare_chart_for_analysis`. `CopilotActionBlock` renders reason `summary` plus compact rows; Accept/Reject and `confirmationToken` wiring unchanged. Proposed AI drawing Accept/Dismiss stays on the chart toolbar — no parallel in-chat Accept for `add_drawing`. NDJSON stream unchanged; blocks in-memory only; SEC-06 / SEC-20 untouched.

**Copilot chat blocks (Phase 6 — Trace chips + answer composition):** `formatTraceDisclosureLabel` drives Thoughts summary (`Thinking` / `Thought for Ns`). Expandable Trace tool chips show action+target with kind icons and `+K more` overflow. In-memory `startedAtMs` / `thoughtDurationSec` on assistant messages (not persisted). `copilot-answer-compose` wraps prose, streaming cursor, labeled Sources (`CopilotReferenceBlock labeled`), labeled Follow-ups, and message actions. NDJSON stream and confirm gates unchanged.

**Quant research runtime (Phase 5 — compare, pin, promote):** Extends Phase 4 with `compare_research_runs` (2–5 jobId/runFingerprint refs → side-by-side KPIs, parameter diffs, warnings), `export_research_draft` (manual copy-only signal/strategy draft from succeeded runs — no auto script/playbook mutation), and `researchRun` Research Board pins with jobId/runFingerprint provenance. Job records persist capped `toolInput` for compare diffs (`COMPUTE_VERSION` 1.4.0). Server-only registry tools remain outside `CLIENT_AI_TOOLS`; permission `read`. Compact results feed Copilot Data blocks (`researchProfile` / `researchCompare` hints). Full series stay in artifacts under `data/research/`. Domain: [src/lib/researchCompute/ARCHITECTURE.md](../researchCompute/ARCHITECTURE.md). Roadmap: [docs/roadmaps/quant-research-runtime-roadmap.md](../../../docs/roadmaps/quant-research-runtime-roadmap.md).
...
- [src/lib/researchCompute/ARCHITECTURE.md](../researchCompute/ARCHITECTURE.md) — Quant research compute domain (server-side experiments; Phase 0 contracts)
