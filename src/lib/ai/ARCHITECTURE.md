...
**Quant research runtime (Phase 3 — strategy evaluation):** Server-side experiment compute ships dataset materialization, descriptive profile jobs, signal / event studies, and minimal vectorized strategy evaluation via `src/lib/researchCompute/` and server-only registry tools in `src/lib/ai/tools/researchCompute.ts` (`create_research_dataset`, `get_research_dataset`, `profile_research_dataset`, `run_signal_study`, `run_strategy_evaluation`, `get_research_job`, `get_research_artifact`). Tools are **not** in `CLIENT_AI_TOOLS`; permission `read`; inputs are dataset refs + typed specs — never raw OHLCV from the model. `run_strategy_evaluation` requires fees, slippage, and fill timing. `ResearchComputePort` is injected in `createServerToolContext()`. Compact tool results feed Copilot Data blocks (`researchProfile` hint); equity curves and full trade tables stay in artifacts under `data/research/`. Domain: [src/lib/researchCompute/ARCHITECTURE.md](../researchCompute/ARCHITECTURE.md). Roadmap: [docs/roadmaps/quant-research-runtime-roadmap.md](../../../docs/roadmaps/quant-research-runtime-roadmap.md).
