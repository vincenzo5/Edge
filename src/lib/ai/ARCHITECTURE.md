...
**Quant research runtime (Phase 0 — contracts):** Server-side experiment compute (datasets → signal studies → vectorized eval → sandboxed Python) is a separate track from chart tools and indicator scripting. Planned registry tools (`create_research_dataset`, `profile_research_dataset`, etc.) are **server-only** (not `CLIENT_AI_TOOLS`); permission `read`; inputs are dataset refs + typed specs — never raw OHLCV from the model. Compact tool results feed Copilot Data blocks; full series stay in artifacts. Domain stub: [src/lib/researchCompute/ARCHITECTURE.md](../researchCompute/ARCHITECTURE.md). Roadmap: [docs/roadmaps/quant-research-runtime-roadmap.md](../../../docs/roadmaps/quant-research-runtime-roadmap.md).
