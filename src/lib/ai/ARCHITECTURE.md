...
**Quant research runtime (Phase 1 — datasets + profile):** Server-side experiment compute ships dataset materialization and descriptive profile jobs via `src/lib/researchCompute/` and server-only registry tools in `src/lib/ai/tools/researchCompute.ts` (`create_research_dataset`, `get_research_dataset`, `profile_research_dataset`, `get_research_job`, `get_research_artifact`). Tools are **not** in `CLIENT_AI_TOOLS`; permission `read`; inputs are dataset refs + typed specs — never raw OHLCV from the model. `ResearchComputePort` is injected in `createServerToolContext()`. Compact tool results feed Copilot Data blocks (`researchProfile` hint); full series stay in Parquet artifacts under `data/research/`. Domain: [src/lib/researchCompute/ARCHITECTURE.md](../researchCompute/ARCHITECTURE.md). Roadmap: [docs/roadmaps/quant-research-runtime-roadmap.md](../../../docs/roadmaps/quant-research-runtime-roadmap.md).
