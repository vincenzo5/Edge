...
- **Ticket layout (compose):** TradingView-style stack — symbol row includes **Risk policy** picker (user templates; hover = recipe). `BuySellToggle`, order-type tabs (**Market / Limit / Stop / Trail**), then an `EdgeMicroSelect` modifier fixed at the left of the price block — **Fill** (Now / Close) on Market/Limit or **Type** (Market / Limit) on Stop/Trail. Market puts the micro select on the Order Price row; Limit/Stop/Trail put it just above the first price field. Composes to the eight IB `orderType` values below. Quantity, linked TP/SL editor (Market/Limit entry only) with **per-leg Qty** plus Offset/Price/USD/%, runner strip when policy scales at TP. Time in Force (Day/GTC/IOC/OPG where valid) + AON + Extended hours. **Price mgmt algo** on limit-like types. **Advanced** when bracket on (non-policy): stop leg + Manage preset. **Review** + side-tinted CTA → preview → confirm.
