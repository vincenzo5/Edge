...
- **Ticket layout (compose):** TradingView-style stack — `BuySellToggle` (Sell | last | Buy, center last pill), `EdgeUnderlineTabs` order types (Market / Limit / Stop / Stop Limit), type-dependent order price fields, quantity, linked TP/SL editor (`Offset` / `Price` / `USD` / `%`; seeded from drawing, compose source of truth), Time in Force + Extended hours. **Advanced** (default collapsed) when bracket on: stop leg (Fixed/Trail) + Manage preset. **Review** immediately above summarizing CTA (`BUY 1 SYM @ price TYPE`) with notional, margin, risk/reward; Risk plan teaser folds into Review. Side-tinted CTA → preview → inline confirm. Preview/submit payloads unchanged (`OrderDraft` / `BracketPlan` → sidecar).
...
Shared vocabulary for every risk surface — full slot taxonomies and UX-moment phases live in the [Risk Management System Roadmap](../../../docs/roadmaps/risk-management-system-roadmap.md). Persisted Zod spine + Postgres columns: `src/lib/risk/policy/` (Phase 1–3). `applyRiskPolicy` creates `planned` instances with reject/swap conflict policy; `evaluatePlaybooks` promotes due schedules and evaluates `managedApp` exits only. **Phase 4 (shipped):** Application settings **Risk policies** tab (`RiskPoliciesSection.tsx`) + sectioned `PlaybookTemplateEditor` with completeness strip; template slot PATCH via `playbookTemplateMutations.ts`. **Phase 5 (shipped):** Chart Plan panel policy apply (`PositionPlanPanel` + `usePositionPlanPolicy`); Trade ticket seeds from planned instance with LMT default, EntrySchedule, live Protect gate, and promote/arm via `/api/trading/playbooks/apply` + `/planned`. View-only compose: `composeRiskPolicyView.ts` (Phase 9).
