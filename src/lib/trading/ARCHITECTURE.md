...
Picker shows Gateway-discovered paper/live accounts only. When live discovery fails, `TWS_LIVE_ACCOUNT_ID` seeds one offline live row (`availability: offline`, label `(live, offline)`) for journal filter — trading remains disabled. When `EDGE_DEMO_JOURNAL_ACCOUNT_ID` is set, `listAccounts` also appends an offline **Demo** paper row (or returns it alone when sidecar/Gateway is down). Legacy `connectionId: journal` selections remap to gateway/offline live by `accountId` on load.
...
**Open-risk header chrome:** When live IB `positions.length > 0`, `AppTopHeader` shows a flat open-positions status chip (`● N open · ±$unrealized`, no border-legend label) with green/red tone from unrealized P&L. The chip opens a positions popover (Close + chart load per row; **Open Account** + **Journal opens** footers). Each row includes a compact economics line (`Avg · Last · Notional · Cost · Risk · % NLV`) from `openPositionEconomics` + resting stop via `resolveOpenPositionProtectStop`; unprotected rows show `Risk —` (never $0). Popover header shows account **Excess · Maint** from summary tags — not per-position margin. Protective OCO stays in the Account panel only. Source of truth is `AccountProvider` only — not journal open trades. Workspace registers `OpenRiskWorkspaceBridge` so Account/chart actions work from the header; off-workspace routes queue pending sidebar/symbol via `pendingWorkspaceActions`. Account rail shows a count badge; command palette entry **Open positions** when the chip is visible. Chart-native order management remains separate backlog.
...
- **Ticket layout (compose):** TradingView-style stack — symbol row includes **Risk policy** picker (user templates; hover = recipe). `BuySellToggle`, order-type tabs (**Market / Limit / Stop / Trail**), then an `EdgeMicroSelect` modifier — **Fill** (Now / Close) on Market/Limit or **Type** (Market / Limit) on Stop/Trail. Market/MOC omit a redundant Order Price readout (last is on Buy/Sell); Limit/Stop/Trail place the modifier above the first price field. Composes to the eight IB `orderType` values below. Quantity, linked TP/SL editor (Market/Limit entry only) with **per-leg Qty** plus Offset/Price/USD/%, runner strip when policy scales at TP. Time in Force (Day/GTC/IOC/OPG where valid) + AON + Extended hours. **Price mgmt algo** on limit-like types. **Advanced** when bracket on (non-policy): stop leg + Manage preset. Sticky **Review** header with notional, max size, qty/max capacity bar, available-after, and risk-to-stop (no compose reward/R:R/risk plan) + side-tinted CTA → preview → confirm.
...
**Ticket UX:** Four primary families compose to the eight IB types via `orderTypeFamily.ts`:

| Primary | Secondary | Edge `orderType` |
|---------|-----------|------------------|
| Market | Fill Now / On close | `MKT` / `MOC` |
| Limit | Fill Now / On close | `LMT` / `LOC` |
| Stop | Type Market / Limit | `STP` / `STP LMT` |
| Trail | Type Market / Limit | `TRAIL` / `TRAIL LIMIT` |
