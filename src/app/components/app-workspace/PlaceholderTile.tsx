...
export default function PlaceholderTile({ onAssign }: Props) {
  return (
    <div
      ...
      className="flex h-full min-h-0 flex-col items-center justify-center gap-3 bg-[var(--edge-surface-panel)] p-4"
    >
      ...
      <div className="flex flex-wrap items-center justify-center gap-2">
        {ASSIGNABLE_SURFACES.map((surface) => (
          <button
            ...
            className="rounded border border-[var(--edge-border-subtle)] bg-[var(--edge-surface-toolbar)] px-3 py-1.5 text-xs font-medium text-[var(--edge-text-secondary)] transition-colors hover:bg-[var(--edge-surface-hover)] hover:text-[var(--edge-text-primary)]"
