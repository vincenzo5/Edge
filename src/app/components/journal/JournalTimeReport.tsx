...
type Props = {
  hourRows: BreakdownRow[];
  weekdayRows: BreakdownRow[];
};

export default function JournalTimeReport({ hourRows, weekdayRows }: Props) {
  const [tab, setTab] = useState<"hour" | "weekday">("hour");
  const rows = tab === "hour" ? hourRows : weekdayRows;

  return (
    <section
      data-testid="journal-time-report"
      className="rounded border border-[var(--edge-border)] bg-[var(--edge-surface-panel)] p-3"
    >
      <div className="mb-3 flex items-center justify-between gap-2">
        <h2 className="text-sm font-semibold text-[var(--edge-text-strong)]">Time analysis (ET)</h2>
        <div className="flex gap-1">
          <TabButton active={tab === "hour"} onClick={() => setTab("hour")} testId="journal-time-hour">
            Hour
          </TabButton>
          <TabButton
            active={tab === "weekday"}
            onClick={() => setTab("weekday")}
            testId="journal-time-weekday"
          >
            Weekday
          </TabButton>
        </div>
      ...
      {rows.length === 0 ? (
        <p className="text-sm text-[var(--edge-text-secondary)]" data-testid="journal-time-empty">
          ...
          <table className="min-w-full text-left text-xs">
            ...
            <tbody>
              {rows.map((row) => (
                <tr
                  key={row.bucket}
                  data-testid={`journal-time-row-${row.bucket}`}
