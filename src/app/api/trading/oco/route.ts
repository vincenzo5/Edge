...
import { SubmitProtectiveOcoRequestSchema } from "@/lib/trading/types";
import {
  getTradingService,
  isTradingConfigured,
...
export async function POST(request: Request): Promise<Response> {
  ...
  const parsed = SubmitProtectiveOcoRequestSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
      { error: "Invalid protective OCO submit request", details: parsed.error.flatten() },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const result = await getTradingService().submitProtectiveOco(
      parsed.data.plan,
