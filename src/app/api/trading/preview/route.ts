...
import { OrderDraftSchema } from "@/lib/trading/types";
import {
  getTradingService,
  isTradingConfigured,
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { error: "Invalid order draft", details: parsed.error.flatten() },
      { status: 400 },
    );
  }

  try {
    const result = await getTradingService().previewOrder(parsed.data);
